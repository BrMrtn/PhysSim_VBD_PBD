...
projects positions onto constraint manifolds (so that, for example, each edge is
shared by at most two triangles) through Gauss--Seidel sweeps, and recovers
velocities by finite difference. PBD is unconditionally stable, but it has a
...
iteration per substep}, rather than a single long step with many iterations. With
only one iteration the Lagrange multiplier is always zero, so the need to store
and update it disappears. This version is now a standard XPBD configuration, but
for the sake of a fair comparison I deliberately leave this optimization out of
the core comparison.
...
initialization} seeds each iteration from a blend of the inertial and
previous-step positions. \emph{Graph-coloring parallelism} lets vertices that
...
\begin{enumerate}
    ...
    \item \textbf{Canonical VBD.} Ma et~al.'s pseudocode departs in several
        ...
        follow canonical VBD and disable Chebyshev acceleration and adaptive
        initialization for the core comparison, so that I read off algorithmic
        properties rather than acceleration tricks.
    \item \textbf{Beyond frame rate.} Ma et~al.\ report fps and node counts. I add
        a Newton ground truth, an equal-wall-clock quality frontier, and targeted
        stress tests (mass ratio, stiffness ratio, large $dt$) keyed to the
