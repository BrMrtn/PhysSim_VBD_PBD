...
new scene, drag and drop objects into it (in my example, cloth models were simple plane 
meshes imporetd from Blender), and assign the relevant components to them, which then
decides what solver it will run. It also has several built-in tools that proved really
useful for data collection: the Profiler for gathering FPS statistics, the Recorder for
...
\label{sec:Platform}
%----------------------------------------------------------------------------
As it's probably obvious from the previous chapter, the simulators are implemented inside the Unity Editor.
All measurements reported in \autoref{chap:Experiments} were collected in the following environment:
\begin{itemize}
...
keeping each local Hessian positive-definite let the method handle more extreme
scenarios without noticeably changing its behavior or performance, so I keep each
local Hessian positive-definite.
