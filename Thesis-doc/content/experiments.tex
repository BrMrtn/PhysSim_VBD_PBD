%----------------------------------------------------------------------------
\chapter{Experiments and results}
\label{chap:Experiments}
%----------------------------------------------------------------------------

\begin{itemize}
    \item This chapter reports measurements along the four evaluation axes from \autoref{sec:EvalAxes}: efficiency, robustness, physical realism, flexibility.
    \item Each axis bears on at least one of the hypotheses H1--H3 from \autoref{sec:Hypotheses}; synthesis is deferred to \autoref{chap:Discussion}.
    \item All experiments use the shared scaffolding and fairness controls of \autoref{sec:Fairness}: matched gravity, $dt = 1/30$~s, matched Rayleigh damping, $\alpha_{\mathrm{XPBD}} = 1/k_{\mathrm{VBD}}$, Chebyshev and adaptive initialisation disabled in VBD for the core comparison.
\end{itemize}

%----------------------------------------------------------------------------
\section{Computational efficiency --- cloth}
\label{sec:EffResults}
%----------------------------------------------------------------------------

\subsection{Frame time vs.\ node count}
\label{ssec:MsVsNodes}
\begin{itemize}
    \item Measure isolated solver time (ms/frame) over a hanging-cloth scene at mesh resolutions \emph{TODO: list resolutions, e.g.\ $10\times 10$ to $100\times 100$}, holding solver settings fixed at the \autoref{sec:Fairness} defaults.
\end{itemize}
\emph{TODO: Figure --- ms/frame vs.\ node count, XPBD and VBD curves.}

\subsection{Frame time vs.\ substep / iteration count}
\label{ssec:MsVsCount}
For a fixed mesh resolution I sweep:
\begin{itemize}
    \item XPBD substep count $n_{\mathrm{sub}} \in \{1, 2, 4, 8, 16, 32\}$ with a single inner iteration (the small-steps configuration),
    \item VBD iteration count $n_{\mathrm{it}} \in \{1, 2, 4, 8, 16, 32\}$.
\end{itemize}
\emph{TODO: Figure --- ms/frame vs.\ work count, both solvers on common axis.}

\subsection{Maximum node counts at 30 and 60~fps}
\label{ssec:MaxNodes}
\begin{itemize}
    \item Following Ma et~al.~\cite{app142311072}, I report the largest mesh sustaining 30~fps and 60~fps for each solver at default settings.
\end{itemize}
\emph{TODO: Table --- max node count, two thresholds, two solvers.}

\subsection{Convergence: residual vs.\ work}
\label{ssec:ResidualConvergence}
\begin{itemize}
    \item For a moderately stretched cloth in isolation, measure the variational residual $\|\nabla G(\mathbf{x})\|$ as a function of iteration count and of wall-clock time, plotting both solvers against a Newton reference.
    \item This is the analogue of Fig.~17 in \cite{Chen2024}.
\end{itemize}
\emph{TODO: Figure --- residual vs.\ iterations and residual vs.\ wall-clock.}

\subsection{Equal-wall-clock quality frontier}
\label{ssec:EqualWallClock}
\begin{itemize}
    \item The headline efficiency result.
    \item Compute solution error (vs.\ Newton) at every combination of $n_{\mathrm{sub}}$ (XPBD) and $n_{\mathrm{it}}$ (VBD), and plot error against ms/frame on a log--log axis.
    \item The lower envelope of the two clouds is each solver's quality frontier; the \emph{equal-wall-clock} comparison reads off which solver achieves lower error at a given ms/frame budget.
\end{itemize}
\emph{TODO: Figure --- error vs.\ ms/frame Pareto fronts.}

\subsection{VBD with Chebyshev and adaptive initialisation (production mode)}
\label{ssec:VBDChebyshev}
\begin{itemize}
    \item For completeness, re-run the equal-wall-clock experiment of \autoref{ssec:EqualWallClock} with VBD's Chebyshev acceleration and adaptive initialisation re-enabled.
    \item This is the configuration a practitioner would deploy.
    \item Reported separately because, per \autoref{ssec:DisableChebyshev}, it does not isolate the primal/dual distinction --- it measures VBD plus its accelerations against vanilla XPBD.
\end{itemize}
\emph{TODO: Figure / table --- accelerated VBD on the same axes as \autoref{ssec:EqualWallClock}.}

%----------------------------------------------------------------------------
\section{Robustness and stability}
\label{sec:Robustness}
%----------------------------------------------------------------------------

\subsection{Stability map: max stable $dt$ vs.\ stiffness}
\label{ssec:StabilityMap}
\begin{itemize}
    \item For each solver, sweep $dt$ and material stiffness $k$, flagging combinations where artefacts appear (oscillation amplitude exceeds rest length, NaNs, visible blow-up).
    \item Deliverable: a 2D stability map per solver, summarising at a glance each solver's operating envelope.
    \item This replaces the simpler ``raise $dt$ until artefacts appear'' test of \cite{app142311072} (their Figs.~7--8) with a fuller picture.
\end{itemize}
\emph{TODO: Figure --- 2D stability map for XPBD and VBD on shared axes.}

\subsection{Single-iteration regime}
\label{ssec:SingleIter}
\begin{itemize}
    \item Both solvers configured for exactly one inner iteration per substep; report whether each remains visually plausible across the default scenes.
    \item This is the most adversarial setting for each algorithm and isolates per-iteration update quality.
\end{itemize}
\emph{TODO: Side-by-side renders, qualitative description.}

\subsection{High mass ratio at the end of a pendulum}
\label{ssec:MassRatioPendulum}
\begin{itemize}
    \item A single spring pendulum with a heavy weight at the free end and a light chain leading to the pivot.
    \item Sweep the mass ratio $m_{\mathrm{tip}} / m_{\mathrm{chain}}$ from $1$ to $10^{4}$; report each solver's qualitative behaviour and quantitative position error vs.\ Newton.
    \item \emph{Hypothesis H1 prediction: XPBD degrades.}
\end{itemize}
\emph{TODO: Figure --- error vs.\ mass ratio.}

\subsection{High mass ratio chain}
\label{ssec:MassRatioChain}
\begin{itemize}
    \item A chain with alternating heavy and light particles, designed to maximise the chain length over which mass disparities couple.
    \item Same outputs as the pendulum.
\end{itemize}
\emph{TODO: Figure / description.}

\subsection{High stiffness ratio chain}
\label{ssec:StiffnessRatioChain}
\begin{itemize}
    \item A chain with alternating stiff and soft springs; sweep the stiffness ratio from $1$ to $10^{4}$.
    \item \emph{Hypothesis H2 prediction: VBD degrades.}
\end{itemize}
\emph{TODO: Figure --- error vs.\ stiffness ratio.}

\subsection{Self-collision: cloth folded onto itself}
\label{ssec:SelfCollResults}
\begin{itemize}
    \item A cloth dropped onto a small obstacle, folding onto itself.
    \item Qualitative visual comparison, noting that the contact model differs (projection vs.\ penalty) per \autoref{sec:SelfCollision}.
\end{itemize}
\emph{TODO: Renders, qualitative description.}

%----------------------------------------------------------------------------
\section{Physical realism}
\label{sec:Realism}
%----------------------------------------------------------------------------

\subsection{Newton reference}
\label{ssec:NewtonRef}
\begin{itemize}
    \item Build a high-iteration Newton solver targeting $\|\nabla G\| < 10^{-9}$ (or until further iterations reduce $G$ by less than $10^{-12}$).
    \item The Newton solver shares storage and energies with the production solvers, so the comparison is over algorithm, not material.
    \item Error metric throughout: mass-weighted position error $\|\mathbf{x} - \mathbf{x}^{\star}\|_{\mathbf{M}}$, where $\mathbf{x}^{\star}$ is the Newton solution.
\end{itemize}

\subsection{Harmonic oscillator}
\label{ssec:Harmonic}
\begin{itemize}
    \item A single spring-mass system oscillating in 1D, compared against the analytic solution $x(t) = A\cos(\omega t - \phi)\,e^{-\zeta\omega t}$.
    \item I measure:
    \begin{itemize}
        \item period as a function of $h$ and of iteration / substep count,
        \item amplitude decay rate as a function of damping coefficient.
    \end{itemize}
    \item This is the analogue of Fig.~2 in \cite{10.1145/2994258.2994272}.
\end{itemize}
\emph{TODO: Figure --- measured period and decay, both solvers, against analytic.}

\subsection{Energy tracking}
\label{ssec:EnergyTracking}
\begin{itemize}
    \item For an undamped free-falling and bouncing cloth, plot total mechanical energy over time.
    \item Identify systematic energy gain (instability) or loss (numerical damping) for each solver.
\end{itemize}
\emph{TODO: Figure --- energy vs.\ time, both solvers.}

\subsection{Hanging cloth: stretched area vs.\ reference}
\label{ssec:HangingCloth}
\begin{itemize}
    \item A square cloth pinned at the top corners, allowed to settle.
    \item Measure steady-state stretched area against the Newton reference.
    \item A realism check at static equilibrium, where transient dynamics drop out.
\end{itemize}
\emph{TODO: Figure --- stretched area error vs.\ stiffness.}

\subsection{Damping cross-check}
\label{ssec:DampingCheck}
\begin{itemize}
    \item At matched Rayleigh coefficients $\alpha_{\mathrm{Rayleigh}}$ and $\beta_{\mathrm{Rayleigh}}$, measure the amplitude-decay curve produced by each solver on the harmonic oscillator.
    \item Identical decay confirms the damping models of \autoref{sec:Damping} are calibrated consistently; this both validates fairness and is a result in its own right.
\end{itemize}
\emph{TODO: Figure --- decay envelopes overlaid.}

%----------------------------------------------------------------------------
\section{Flexibility}
\label{sec:FlexResults}
%----------------------------------------------------------------------------

\subsection{Iteration-count independence of stiffness}
\label{ssec:IterIndepStiff}
\begin{itemize}
    \item For each solver, sweep stiffness $k$ over several decades and measure how many iterations / substeps are needed to reach a fixed error threshold.
    \item Following Fig.~6 in \cite{10.1145/2994258.2994272}, XPBD is expected to be roughly stiffness-independent in iteration count thanks to the compliance formulation.
\end{itemize}
\emph{TODO: Figure --- iterations to threshold vs.\ stiffness.}

\subsection{Stiffness-ratio test}
\label{ssec:StiffRatioTest}
\begin{itemize}
    \item Same kind of plot as the previous subsection, but varying the \emph{ratio} of stiffnesses across the mesh rather than a uniform stiffness.
    \item This is the experiment of \cite{Chen2024} Fig.~24 (and AVBD~\cite{Giles2025} Fig.~4).
    \item Includes a brief discussion of AVBD's improvement on this axis without implementing it myself.
\end{itemize}
\emph{TODO: Figure --- error vs.\ stiffness ratio, with AVBD reference qualitatively cited.}

\subsection{Ease of adding a new constraint or force}
\label{ssec:NewConstraint}
I implement a representative additional force --- \emph{TODO: choose, e.g.\
bending-angle constraint or area-preservation force} --- in both solvers
and report:
\begin{itemize}
    \item lines of code added,
    \item whether a Hessian derivation was required (VBD: yes; XPBD: no, a gradient suffices),
    \item time to first correct simulation.
\end{itemize}
\begin{itemize}
    \item The metric is admittedly subjective but captures a real practitioner cost that the more quantitative experiments above do not.
\end{itemize}
\emph{TODO: Table --- LOC, derivation effort, time-to-correct.}
